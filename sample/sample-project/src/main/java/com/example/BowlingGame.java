...
public class BowlingGame {
    ...
    public BowlingGame() {
        ...
        // Initialize 10 frames, marking the last one for special handling
        for (int i = 0; i < 10; i++) {
    ...
    public int score() {
        ...
        for (int i = 0; i < frames.size(); i++) {
            ...
            // Add bonus for strikes and spares (not for the last frame,
            // since the last frame's extra rolls are the bonus itself)
            if (i < 9) {
                if (frame.isStrike()) {
    ...
    private int getStrikeBonus(int frameIndex) {
        ...
        if (frameIndex < 9) {
            ...
            if (nextFrame.getRolls().size() == 0) {
                return 0; // Next frame hasn't been played yet
            }
