...
- **Gradle Integration** — Works with Gradle projects (uses init scripts to force test re-execution)
...
The extension automatically detects your build tool by looking for `build.gradle` or `build.gradle.kts` in the workspace. It searches upward from test files to find the nearest Gradle project root.

### Force Test Execution

Gradle's up-to-date check is bypassed via an init script (`resources/force-tests.init.gradle`) so tests always run, even if sources haven't changed.
...
│   ├── testController.ts            # VS Code Test Controller (discovery, run, debug)
│   ├── types.ts                     # Shared types (BuildTool enum, etc.)
│   └── services/
│       ├── BuildToolService.ts      # Gradle project root detection & command building
...
│       └── TestResultParser.ts      # Gradle console output & XML report parsing
├── images/
│   └── icon.png                     # Extension icon
├── resources/
│   └── force-tests.init.gradle      # Gradle init script to bypass up-to-date checks
├── sample-project/                  # Sample Gradle + Spock project (Java 21, Spock 2.4)
├── .vscode/
...
A sample Gradle project is included in `sample-project/` using **Java 21**, **Groovy 4.0**, and **Spock 2.4**.
...
- `Person.groovy` — Simple POJO with `name` and `age`
- `User.java`, `UserService.java` — In-memory user model and service
- `BowlingGame.java`, `Frame.java`, `BowlingException.java` — Bowling game engine (main source in `src/main/java`)
...
- **Extension Tests** — Runs extension integration tests

## Troubleshooting

| Problem | Solution |
|---|---|
| Tests not discovered | Ensure test classes extend `Specification` and Gradle is configured correctly |
